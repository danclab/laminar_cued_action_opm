...
OUTPUT_TSV = op.join(BASE_DIR, "fiducial_coords.tsv")


def find_runs(meg_dir):
    files = [f for f in os.listdir(meg_dir) if f.startswith("R") and f.endswith(".tsss.fif")]
...
subjects_dir = "/home/common/bonaiuto/laminar_cued_action_opm/derivatives/fs/"
...
for subject in subjects:
    ...
    sub_dir = op.join(BASE_DIR, subject)
    ...
    for raw_fname, name in zip(run_paths, run_names):

        print(f"\nReading {raw_fname}")

        run_id = name.split(".")[0]

        trans_fname = op.join(sub_dir, "squid", "trans", f"{run_id}-trans.fif")
